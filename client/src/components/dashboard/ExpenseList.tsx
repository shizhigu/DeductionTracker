...
function ExpenseItem({ expense, category, isMobile }: ExpenseItemProps) {
  ...
  if (isMobile) {
    return (
      <Link href={`/expenses/${expense.id}`} className="block">
        <div className="bg-white rounded-xl p-3 border border-neutral-100 shadow-sm flex items-center cursor-pointer">
  ...
  return (
    <Link href={`/expenses/${expense.id}`} className="block">
      <div className="bg-white rounded-xl p-4 border border-neutral-100 shadow-sm flex items-center cursor-pointer">
